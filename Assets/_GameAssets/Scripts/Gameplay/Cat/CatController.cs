...
public class CatController : MonoBehaviour
{
    ...
    private void SetRandomDestination()
    {
        ...
        while (attemps < _maxDestinationAttemps && !destinationSet)
        {
            ...
            if (NavMesh.SamplePosition(randomDirection, out NavMeshHit hit, _patrolRadius,
            NavMesh.AllAreas))
            {
        ...
        if (!destinationSet)
        {
            Debug.LogWarning("Failed to find a valid destionation");
            _isWaiting = true;
            _timer = _waitTime * 2;
        }
    }
    ...
    {
        if (NavMesh.Raycast(transform.position, position, out NavMeshHit hit,
        NavMesh.AllAreas))
        {
            return true;
        }
